...
class audio_out_serializer extends Module {
  val io = IO(new Bundle {
    ...
    // Bidirectionals
    // Outputs
    val left_channel_fifo_write_space = Output(UInt(8.W))
    val right_channel_fifo_write_space = Output(UInt(8.W))
  ...
  val left_channel_fifo_write_space = RegInit(0.U(8.W))
  left_channel_fifo_write_space := 0x80.U(8.W) - Cat(left_channel_fifo_is_full, left_channel_fifo_used)
  io.left_channel_fifo_write_space := left_channel_fifo_write_space

  val right_channel_fifo_write_space = RegInit(0.U(8.W))
  right_channel_fifo_write_space := 0x80.U(8.W) - Cat(right_channel_fifo_is_full, right_channel_fifo_used)
  io.right_channel_fifo_write_space := right_channel_fifo_write_space
  ...
  left_channel_fifo_used := Audio_Out_Left_Channel_FIFO.io.count
  ...
  right_channel_fifo_used := Audio_Out_Right_Channel_FIFO.io.count
